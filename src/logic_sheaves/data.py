...
from .logic import OPS, Expr, random_expr
...
def encode(expression: Expr) -> list[int]:
    return [CLS_ID, *(TOKEN_TO_ID[token] for token in expression.prefix_tokens())]


def collate_expressions(expressions: Sequence[Expr]) -> EncodedBatch:
...
class ExpressionDataset(Dataset[Expr]):
    def __init__(self, expressions: Sequence[Expr]) -> None:
